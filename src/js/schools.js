...
export const SRI_LANKAN_SCHOOLS = [
  ...
  // ==========================================
  // WESTERN PROVINCE - KALUTARA DISTRICT (20+)
  // ==========================================
  ...
  // ==========================================
  // CENTRAL PROVINCE - KANDY DISTRICT (25+)
  // ==========================================
  ...
  // ==========================================
  // CENTRAL PROVINCE - MATALE & NUWARA ELIYA (15+)
  // ==========================================
  ...
  // ==========================================
  // SOUTHERN PROVINCE - GALLE DISTRICT (20+)
  // ==========================================
  ...
  // ==========================================
  // NORTHERN PROVINCE - JAFFNA & DISTRICTS (25+)
  // ==========================================
  ...
  // ==========================================
  // EASTERN PROVINCE (BATTICALOA, AMPARA, TRINCO) (25+)
  // ==========================================
  ...
  // ==========================================
  // NORTH WESTERN PROVINCE (KURUNEGALA & PUTTALAM) (30+)
  // ==========================================
  ...
  // ==========================================
  // NORTH CENTRAL PROVINCE (ANURADHAPURA & POLONNARUWA) (20+)
  // ==========================================
  ...
  // ==========================================
  // UVA PROVINCE (BADULLA & MONARAGALA) (20+)
  // ==========================================
 ...
 */
export function searchSchools(query, limit = 10) {
...
/**
 * Initialize School Autocomplete on an input element
 * @param {HTMLInputElement} inputEl - Target input field
 * @param {HTMLElement} dropdownContainer - Container element for dropdown
 * @param {Function} onSelect - Callback receiving selected school name (string)
 * @returns {{ destroy: Function, setVal: Function }}
 */
export function initSchoolAutocomplete(inputEl, dropdownContainer, onSelect) {
  if (!inputEl || !dropdownContainer) return { destroy: () => {}, setVal: () => {} };

  let activeIndex = -1;
  let currentMatches = [];
  let isOpen = false;

  // Position and style dropdown container
  dropdownContainer.classList.add('glass-dropdown', 'rounded-xl', 'overflow-hidden', 'shadow-2xl', 'z-50', 'hidden');
  dropdownContainer.style.maxHeight = '280px';
  ...
  const renderDropdown = () => {
    ...
    currentMatches = searchSchools(query, 12);
    dropdownContainer.innerHTML = '';

    if (currentMatches.length === 0) {
      // Fallback custom entry option
      ...
      customItem.innerHTML = `
        ...
        <span>Use custom: <strong class="text-white">"${safeQuery}"</strong></span>
      `;
    ...
    currentMatches.forEach((school, index) => {
      ...
      item.className = `px-3.5 py-2.5 text-xs text-slate-200 cursor-pointer transition-colors flex items-center justify-between border-b border-white/5 ${
        index === activeIndex ? 'bg-indigo-600/30 text-white border-l-2 border-indigo-400' : 'hover:bg-white/10'
      }`;
    ...
    // Also append custom entry at bottom if not exact match
    const exactMatch = currentMatches.some(s => s.name.toLowerCase() === query.toLowerCase());
    if (!exactMatch) {
      const customItem = document.createElement('div');
      ...
      customItem.innerHTML = `<span>Can't find it? Use: <span class="text-indigo-300 font-medium">"${safeQuery}"</span></span>`;
  ...
  const onKeyDown = (e) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (activeIndex >= 0 && activeIndex < currentMatches.length) {
        selectSchool(currentMatches[activeIndex].name);
      } else if (inputEl.value.trim().length > 0) {
        selectSchool(inputEl.value.trim());
      }
  ...
  const onDocClick = (e) => {
    if (!inputEl.contains(e.target) && !dropdownContainer.contains(e.target)) {
      isOpen = false;
      renderDropdown();
    }
  };

  const _scrollActiveIntoView = () => {
    if (activeIndex >= 0) {
      const items = dropdownContainer.querySelectorAll('div[class*="px-3.5"]');
