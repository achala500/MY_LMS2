...
export interface AchievementBadge {
  ...
  description: string;
  icon: string;
  category: 'streak' | 'hours' | 'mastery' | 'habit';
  unlocked: boolean;
...
/**
 * Calculate total XP earned by a student:
 * - 100 XP per logged study hour
 * - 50 XP bonus per active streak day
 * - 25 XP bonus per recorded study session
 */
export function calculateXp(totalHours: number, streak: number, sessionCount: number): number {
...
/**
 * Compute Level and progress towards next tier
 */
...
/**
 * Check whether any study session qualifies as Early Bird or Night Owl
 */
function inspectSessionHabits(logs: DailyLogEntry[]): { hasEarlyBird: boolean; hasNightOwl: boolean } {
  let hasEarlyBird = false;
  let hasNightOwl = false;

  for (const log of logs) {
    // 1. Check timestamp hour if available
    if (log.timestamp) {
      try {
    ...
    // 2. Check notes content for habit keywords
...
/**
 * Evaluate all 8 required standard achievement badges
 */
...
/**
 * Get comprehensive gamification state rollup
 */
