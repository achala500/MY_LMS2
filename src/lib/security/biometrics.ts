...
export async function hashString(str: string): Promise<string> {
  // Check universal globalThis.crypto or window.crypto for standard Web Crypto API
  ...
  if (!cryptoObj || !cryptoObj.subtle) {
    // Basic fallback hash for non-standard environments lacking Web Crypto API
    let hash = 0;
